...
CREATE OR REPLACE PROCEDURE create_post(header citext,
                                        short_topic citext,
                                        main_topic citext,
                                        authors citext[],
                                        username citext)
...
BEGIN
    ...
    INTO STRICT userID
    FROM users
    WHERE LOGIN = create_post.username;

EXCEPTION
    WHEN NO_DATA_FOUND THEN RAISE EXCEPTION 'User % not found', create_post.username;
...
CALL create_post('kek_header', 'kek2', 'kek4', '{"e", "w", "d"}', 'smet_k_121');
-- ок
CALL create_post('kek_header', 'kek2', 'kek4', '{"e", "w", "d"}', 'smet_k_2');
...
UPDATE
    posts p
SET header = 'test_header_2_updated'
WHERE id = 4
RETURNING
    p.*;
...
CREATE OR REPLACE PROCEDURE create_valid_post(header citext,
                                              short_topic citext,
                                              main_topic citext,
                                              authors citext[],
                                              username citext)
...
BEGIN
    ...
    INTO STRICT userID
    FROM users
    WHERE LOGIN = create_valid_post.username;
    IF NOT FOUND THEN
...
-- ok
CALL create_valid_post('kek_t_4', 'kek1_t_5', 'kek2_t_6', '{"a", "b", "c e"}', 'smet_k');


SELECT array_length(p.authors, 1),
       p.*
FROM posts p;

UPDATE
    posts p
SET p.authors = array_append(p.authors,
                             SELECT
                             LOGIN FROM users
                             WHERE
                             id = p.user_id);
